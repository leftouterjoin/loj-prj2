...
ls.ListView = Class.create( {
  ...
  //RESIZE_COLUMN_HEADER : 1,
	...
	_buildTable: function() {
		// カラム定義、ヘッダにダミー追加
		var _cols = this._area.getElementsBySelector('col');
		var i = 0;
		for (i = 0; i < _cols.length; i++) {
			_cols[i].addClassName(this._areaId + '_col_' + i);
		}
		new Insertion.After(_cols[i - 1], '<col class="_col_dummy_" />');
		var _thead = this._area.getElementsBySelector('thead');
		var _tbody = this._area.getElementsBySelector('tbody');
		var _theadCols = _thead[0].getElementsBySelector('th');
		new Insertion.After(_theadCols[i - 1], '<th class="_col_dummy_"></th>');
		// thead、tbody のラッパーを生成
		new Insertion.Bottom(this._area, '<div id="' + this._areaId + '_thead_wrap' + '" style="position:relative;display:none;overflow:hidden;"></div>');
		new Insertion.Bottom(this._area, '<div id="' + this._areaId + '_tbody_wrap' + '" style="position:relative;display:none;overflow-y:scroll;"></div>');
		this._hwrap = $(this._areaId + '_thead_wrap');
		this._bwrap = $(this._areaId + '_tbody_wrap');
		// tableをラッパーに追加
		var _table = this._area.getElementsBySelector('table');
		this._hwrap.appendChild(_table[0].clone());
		this._bwrap.appendChild(_table[0].clone());
		// 各ラッパーにカラム定義を追加(bodyにはダミーを追加しない)
		_cols = this._area.getElementsBySelector('col');
		var _htable = this._hwrap.getElementsBySelector('table');
		var _btable = this._bwrap.getElementsBySelector('table');
		var _resizerPos = 0;
		for (i = 0; i < _cols.length; i++) {
			_htable[0].appendChild(_cols[i].clone());
			//_resizerPos += _cols[i].getWidth();
			//new Insertion.Bottom(this._hwrap, '<div id="' + this._areaId + '_col_resizer_' + i + '" class="col_resizer" style="left:' + (_resizerPos - 4) + 'px"></div>');
			
			//Event.observe($(this._areaId + '_col_resizer_' + i), 'mousedown', this._onResizerMouseDown.bindAsEventListener(this));
			
			if(i < _cols.length - 1) _btable[0].appendChild(_cols[i].clone());
		}
		
		//Event.observe(document.body, 'mousemove', this._onResizerMouseMove.bindAsEventListener(this));
		//Event.observe(document.body, 'mouseup',   this._onResizerMouseUp.bindAsEventListener(this));

		// スクロールイベント
		Event.observe(this._bwrap, 'scroll', this._onScroll.bindAsEventListener(this));

		_htable[0].appendChild(_thead[0]);
		_btable[0].appendChild(_tbody[0]);
		_table[0].remove();
		_tbody = this._area.getElementsBySelector('tbody');
		this._body = _tbody[0];
	...
	setBodyHeight: function(height) {
		this._bwrap.setStyle({'height': height + 'px'});
	},

	getBodyOffset: function() {
		return {'top':this._bwrap.offsetTop, 'left':this._bwrap.offsetLeft};
	},

	_onResizerMouseDown: function(event) {
		if (this.isResize != null) return;
		var _resizer = Event.element(event);
		var _i = _resizer.id.substring(_resizer.id.lastIndexOf('_') + 1);
		var _hcols = this._hwrap.getElementsBySelector('col.' + this._areaId + '_col_' + _i);
		var _bcols = this._bwrap.getElementsBySelector('col.' + this._areaId + '_col_' + _i);
		this.isResize = {'index':_i, 'left':Event.pointerX(event), 'hcol':_hcols[0], 'bcol':_bcols[0]};
	},

	_onResizerMouseMove: function(event) {
		if (this.isResize == null) return;
		
		var _diff = Event.pointerX(event) - this.isResize.left;
		var _w = this.isResize.hcol.getWidth() + _diff;
		this.isResize.hcol.setStyle({'width': _w + 'px'});
		this.isResize.bcol.setStyle({'width': _w + 'px'});
		this.isResize.left = Event.pointerX(event);
		var _resizers = this._hwrap.getElementsBySelector('div.col_resizer');
		for (i = 0; i < _resizers.length; i++) {
			_resizers[i].setStyle({'left':(_resizers[i].offsetLeft + _diff) + 'px'});
		}
		
	},

	_onResizerMouseUp: function(event) {
		if (this.isResize == null) return;
		this.isResize = null;
	},
	
	_onScroll: function(e) {
		this._hwrap.scrollLeft = this._bwrap.scrollLeft;
	},
  ...
  /*
  doLayout: function() {
	var _p = this._area.getOffsetParent();
	var _h = _p.getHeight() - this._bwrap.offsetTop - 2;
	this._bwrap.setStyle({'height': _h + 'px'});
  },*/
